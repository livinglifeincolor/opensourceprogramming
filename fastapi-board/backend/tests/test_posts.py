...
async def test_create_post(client, db_override):
    """유효한 데이터로 게시글을 생성하면 201과 생성된 게시글을 반환해야 한다"""
    db_override(rows=[FAKE_POST])
...
async def test_list_posts_empty(client, db_override):
...
async def test_get_post(client, db_override):
...
async def test_get_post_not_found(client, db_override):
...
async def test_update_post_not_found(client, db_override):
...
async def test_delete_post(client, db_override):
    """존재하는 게시글 삭제 시 204를 반환해야 한다"""
    db_override(execute_result="DELETE 1")
...
async def test_delete_post_not_found(client, db_override):
    """존재하지 않는 게시글 삭제 시 404를 반환해야 한다"""
    db_override(execute_result="DELETE 0")
...
async def test_get_posts_count(client, db_override):
